...
import { getFriends, getPendingRequests, respondToFriendRequest, sendFriendRequest, getMyReunionInvites, respondToReunionInvite } from "@/lib/actions/friend.actions";
...
export function FriendsManager({ currentUser }: { currentUser: Player }) {
    ...
    const handleReunionInvite = async (id: string, status: "accepted" | "rejected") => {
        ...
        <Card className="bg-card/40 backdrop-blur border-primary/10">
            ...
            <CardContent className="space-y-6">
                ...
                <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={handleAddFriend} disabled={loading || !email} size="icon">
                        ...
                        {pending.map((r: FriendRequest) => (
                            <div key={r._id} className="flex items-center justify-between p-2 rounded-lg bg-primary/5 border border-primary/10">
                                ...
                                <div className="flex gap-1">
                                    <Button size="icon" variant="ghost" className="h-8 w-8 text-green-500" onClick={() => handleRespond(r._id, "accepted")}>
                                        <UserCheck className="h-4 w-4" />
                                    </Button>
                                    <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleRespond(r._id, "rejected")}>
                    ...
                    <div className="space-y-3">
                        ...
                        {reunionInvites.map((ri: ReunionInvite) => (
                            <div key={ri._id} className="p-3 rounded-lg bg-purple-500/10 border border-purple-500/20 space-y-2">
                                ...
                                <div className="flex gap-2">
                                    <Button size="sm" className="flex-1 bg-purple-600 hover:bg-purple-700 h-8" onClick={() => handleReunionInvite(ri._id, "accepted")}>
                                        Join
                                    </Button>
                                    <Button size="sm" variant="outline" className="h-8" onClick={() => handleReunionInvite(ri._id, "rejected")}>
                                        Ignore
                                    </Button>
                                </div>
                            </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        ...
                        {friends.length === 0 && (
                            <p className="text-center text-xs text-muted-foreground py-4">No friends yet. Add some!</p>
                        )}
                    </div>
                </div>
            </CardContent>
